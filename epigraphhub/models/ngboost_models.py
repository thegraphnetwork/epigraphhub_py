...
def rolling_predictions(target_name, data, ini_date = '2020-03-01',split = 0.75, horizon_forecast = 14, maxlag=14):
...
def train_eval_single_canton(target_curve_name, canton, predictors, vaccine = True, smooth= True,ini_date = '2020-03-01', updated_data = True):
    ...
    * For the predictor hospCapacity is used as predictor the column ICU_Covid19Patients
    ...
    clusters, all_regions,fig = compute_clusters('switzerland', 'cases',
    ...
    # getting the data
    df = get_cluster_data(predictors, list(cluster_canton), vaccine=vaccine, smooth = smooth)
...
def train_and_evaluate_all_cantons(target_curve_name,  predictors, vaccine = True, smooth= True, ini_date = '2020-03-01', title = None):
...
def training_model(target_name, data, ini_date, horizon_forecast = 14, maxlag=14):
    ...
    if np.sum(target) > 0.0: 
        ...
        for T in range(1, horizon_forecast + 1):
            ...
            dump(model, f'../opt/models/saved_models/ml/ngboost_{target_name}_{T}D.joblib')
...
def rolling_forecast(target_name, data, ini_date, horizon_forecast = 14, maxlag=14):
    ...
    #         print(df_lag.index,fit_target.index)
        ...
        for T in range(1, horizon_forecast + 1):
            ...
            model = load(f'../opt/models/saved_models/ml/ngboost_{target_name}_{T}D.joblib')
...
def forecast_single_canton(target_curve_name, canton, predictors, vaccine = True, smooth= True, ini_date = '2020-03-01', title = None, updated_data = True):
...
def forecast_all_cantons(target_curve_name, predictors, vaccine = True, smooth= True,ini_date = '2020-03-01', title = None):
    '''
...
def train_single_canton(target_curve_name, canton, predictors, vaccine = True, smooth= True,ini_date = '2020-03-01', updated_data = True):
...
def train_all_cantons(target_curve_name, predictors, vaccine = True, smooth= True,ini_date = '2020-03-01'):
    '''
